...
def download(url, dir, filename=None, expect_size=None):
    ...
    if os.path.isfile(fpath):
        if expect_size is not None and os.stat(fpath).st_size == expect_size:
            print("File %s exists! Skip download."%filename)
            return fpath
        else:
            print("File %s exists. Will overwrite with a new download!"%filename)
    ...
    if expect_size is not None and size != expect_size:
        print("File downloaded from %s does not match the expected size!"%url)
        print("You may have downloaded a broken file, or the upstream may have modified the file.")

    # TODO human-readable size
    print('Succesfully downloaded %s. %s bytes.'%(filename,size))
    return fpath
...
def maybe_download_and_extract(dest_directory):
    ...
    if os.path.isdir(os.path.join(dest_directory, cifar_foldername)):
        print("Found cifar10 data in %s."%dest_directory)
        return
    else:
        DATA_URL = DATA_URL_CIFAR_10
