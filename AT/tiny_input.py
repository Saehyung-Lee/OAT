"""
Utilities for importing the OOD dataset.
...
class OODData(object):
    def __init__(self, path):
        train_filenames = path 
        train_images, train_labels = self._load_datafile(train_filenames)
    ...
    def _load_datafile(filename):
      with open(filename, 'rb') as fo:
          ...
          target_data = data_dict['extrapolated_targets']
          return image_data, target_data

class AugmentedOODData(object):
    def __init__(self, raw_cifar10data, sess, model):
        assert isinstance(raw_cifar10data, OODData)
        self.image_size = 32
